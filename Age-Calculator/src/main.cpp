...
double gemAge, humanAge, dev, accRange, acc;
...
int doHuman(double humanAge)
{
    ...
    for(double x = 1; x <=100000/accRange;x++) //total age is x/100 so right now it's 100,000
    {
        ...
        if(y >= dev-accRange && y <= dev+accRange) //the lower the o.o1 number is, the more accurate it is, but make s8re to make x bigger too
        {
            gemAge = x*accRange;
            cout << "\nHuman Age: " << humanAge << "\nMinimum Gem Age: " << gemAge*accRange << endl;
            system("pause");
            return 1;
        }
        else
